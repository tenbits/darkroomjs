...
;(function(window, document, fabric) {
  ...
  Darkroom.prototype = {
    ...
    toDataURL: function(options) {
      return this.toCanvas().toDataURL(options);
    },
    toCanvas: function(){
      if (this.source != null) {
        return Darkroom.imgToCanvas(this.source);
      }
      
      var canvas = this.image.canvas;
      canvas.renderAll(true);
      return canvas.upperCanvasEl || canvas.lowerCanvasEl;
    },
    toBlob: function(cb, mime, compression){
      this.toCanvas().toBlob(function(){
        cb(blob, canvas);
      }, mime, compression);
    }
